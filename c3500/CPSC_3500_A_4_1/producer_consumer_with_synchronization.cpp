...
/**Mutex:
 * data_mutex:  for accessing data to keep it clean
 * console_mutex: for accessing the console to keep input and output clean
 * consumer_empty_mutex: for stopping the consumer thread from trying to access orders when there are none
 * Producer_full_mutex: for stopping the producer from adding more to the queue when the queue is already full
*/

pthread_mutex_t data_mutex = PTHREAD_MUTEX_INITIALIZER, console_mutex = PTHREAD_MUTEX_INITIALIZER, consumer_empty_mutex = PTHREAD_MUTEX_INITIALIZER, producer_full_mutex = PTHREAD_MUTEX_INITIALIZER;

/**Conditions:
 * producer_full_cond: for stopping the producer from adding more to the queue when its full
 * Consumer_empty_cond: for stopping the consumer from taking stuff out of the queue while its empty
*/

pthread_cond_t producer_full_cond = PTHREAD_COND_INITIALIZER, consumer_empty_cond = PTHREAD_COND_INITIALIZER;
...
void* takeOrders(void* arg)
{
    ...
    for(int i = 0; i < MAX; ++i) {
        //Check to see if the buffer is empty or not
        if(num_new_orders >= N)pthread_cond_wait(&producer_full_cond, &producer_full_mutex);
        ...
        pthread_cond_signal(&consumer_empty_cond);
...
void* processOrders(void* arg)
{
    ...
    for(int i = 0; i < MAX; ++i) {
        ...
        if(num_new_orders == 0)pthread_cond_wait(&consumer_empty_cond, &consumer_empty_mutex);
        ...
        pthread_cond_signal(&producer_full_cond);
